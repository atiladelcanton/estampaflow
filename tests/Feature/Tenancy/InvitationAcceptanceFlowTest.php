...
it('keeps the invitation page public', function (): void {
    ...
    $result = app(InviteTenantUserAction::class)->execute(
        $tenant,
        $owner,
        'nova@convite.test',
        TenantRole::USER,
    ...
    $this->get($result->acceptUrl)
        ->assertOk()
        ->assertSee('Criar conta e entrar');
});
...
it('registers a new invited user without creating another tenant', function (): void {
    ...
    $registered = app(RegisterInvitedUserAction::class)->execute(
        $result->plainToken,
        'Nova Pessoa',
        'password',
    );
