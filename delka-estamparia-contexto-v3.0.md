...
## ATUALIZAÇÃO V2.4 — ONBOARDING E ACESSOS

- Cadastro público cria automaticamente a estamparia, domínio, Owner e trial.
- Usuário é direcionado diretamente ao tenant.
- Dashboard central é exclusivo do Platform Admin.
- Convite público permite login de usuário existente ou cadastro contextual de usuário novo.
- Cadastro por convite não cria outro tenant.
- Envio de convite registra sucesso/falha em log e auditoria.
- ADR 0011 é normativo.


---

## 26. INFRAESTRUTURA ASSÍNCRONA — SPRINT 1

- fila inicial: `database`;
- filas: `provisioning`, `mail` e `default`;
- Mailpit no desenvolvimento local;
- convites enviados por job criptografado após commit;
- provisionamento de domínio possui estados PENDING, PROCESSING, PROVISIONED e FAILED;
- produção usa DNS wildcard;
- hospedagem compartilhada executa worker de curta duração por cron;
- falhas ficam em `failed_jobs`, logs e AuditLog.


---

## 27. E-MAIL DE BOAS-VINDAS — SPRINT 1

- cadastro público enfileira `SendTenantWelcomeEmailJob` após commit;
- o envio usa a fila `mail` e Mailpit no desenvolvimento;
- a mensagem contém nome da estamparia, URL exclusiva, e-mail de acesso e fim do trial;
- a senha nunca é enviada ou recuperada em texto;
- existe link de redefinição de senha;
- o template HTML segue a paleta e o estilo visual do EstampaFlow;
- existe alternativa em texto puro;
- eventos `queued`, `sent` e `failed` são registrados em log e AuditLog.


---

## ADENDO v2.7 — SPRINT 2 IMPLEMENTADA

O catálogo dinâmico de serviços está implementado com `ServiceType`, `ServiceTypeSchemaVersion` e `ServiceParameterDefinition`.

Regras vigentes:

- DTF, Silk, Sublimação e Bordado são defaults tenant-aware;
- todo novo tenant recebe esses registros no onboarding;
- versões ativas são imutáveis;
- alterações incompatíveis criam um novo draft;
- parâmetros suportam texto, inteiro, decimal, boolean, seleção e multiseleção;
- quantidade aplicada e posição continuam estruturais;
- o motor de preços permanece planejado para a Sprint 3;
- telas do catálogo são restritas ao Owner no MVP.


---

## 18. SPRINT 2.5 — ONBOARDING E CENTRAL DE AJUDA

Status: `IMPLEMENTED`.

- wizard inicial curto e opcional para o Owner;
- escolha dos serviços e convite opcional de equipe;
- tutoriais contextuais por rota e papel;
- progresso por usuário, tenant, chave e versão;
- Central de Ajuda baseada em tarefas;
- botão para rever o tutorial da página;
- auditoria de conclusão e dispensa.

### 18.1 Regra obrigatória de entrega

Toda nova sprint deve revisar o wizard inicial, o tutorial contextual e a Central de Ajuda. A sprint só é concluída após atualizar e testar a orientação correspondente.


---

## Atualização v2.9 — Sprint 3

A Sprint 3 adiciona o motor declarativo e versionado de precificação. Cada serviço pode usar preço por unidade, faixa de quantidade, área, matriz de parâmetros ou faixa de pontos. Valores finais usam Money em centavos; taxas usam DECIMAL(18,8). Regras são isoladas por tenant, vinculadas à versão ativa dos campos, auditadas e preservadas historicamente. Empates retornam AMBIGUOUS. Não existe fórmula livre nem execução de código do tenant.

A experiência do Owner permanece simples: escolher a forma de cobrança, cadastrar regras, salvar e simular. A Sprint 3 também atualiza o wizard opcional, os tutoriais contextuais e a Central de Ajuda.


## Atualização v3.0 — Precificação guiada por operação

A Sprint 3 foi redesenhada para não expor ao Owner estratégias internas, coeficientes ou taxas por cm². A interface principal usa fluxos curtos, ajuda contextual e prévia imediata.

### DTF comprado

- preço informado por metro;
- largura útil do material;
- fornecedor vende somente metros inteiros;
- comprimento necessário é arredondado para cima, com mínimo de um metro;
- o motor testa a arte na posição original e girada;
- a prévia mostra metros estimados, metros comprados, sobra, material, aplicação e preço por peça;
- produção interna de DTF depende do cadastro de insumos da Sprint 4.

### Silk Screen

- tabela comercial por faixa de quantidade e número de cores;
- preço das células sempre representa valor por peça;
- preparação pode estar incluída ou ser cobrada por tela/cor;
- base branca pode contar como cor, adicional por peça ou ficar sem ajuste automático;
- tintas e efeitos são adicionais opcionais;
- frente, costas, manga e outras posições continuam aplicações separadas no orçamento.

### Princípio de UX

O usuário informa como trabalha e quanto cobra. O EstampaFlow transforma os dados em regras versionadas e determinísticas nos bastidores. Opções avançadas ficam recolhidas. Toda tela complexa deve oferecer passo a passo, ajuda inline, modal opcional e resumo antes de salvar.
