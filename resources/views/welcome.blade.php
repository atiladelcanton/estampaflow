<x-layouts.guest title="Delka Estamparia">
    <div class="min-h-screen bg-white">
        <header class="mx-auto flex h-20 max-w-7xl items-center justify-between px-6">
            <x-brand />
            <div class="flex items-center gap-3">
                @auth
                    <a href="{{ route('central.dashboard') }}" class="button-primary">Abrir ambientes <x-icon name="arrow-right" class="size-4" /></a>
                @else
        ...
        <section class="mx-auto grid min-h-[calc(100vh-80px)] max-w-7xl items-center gap-14 px-6 py-16 lg:grid-cols-[1fr_.95fr]">
            <div>
                ...
                <div class="mt-9 flex flex-wrap gap-3">
                    <a href="{{ auth()->check() ? route('central.dashboard') : route('login') }}" class="button-primary px-6 py-3">Explorar a Sprint 1 <x-icon name="arrow-right" class="size-4" /></a>
                    <a href="/up" class="button-secondary px-6 py-3">Verificar saúde</a>
                </div>
                <div class="mt-12 grid max-w-xl grid-cols-3 gap-4">
                    <div><p class="text-2xl font-extrabold">10</p><p class="mt-1 text-xs text-ink-400">ADRs aceitos</p></div>
            ...
            <div class="auth-panel-pattern rounded-[36px] border border-brand-100 p-6 sm:p-10">
                <div class="rounded-[28px] border border-white bg-white p-5 shadow-card">
                    ...
                    <div class="mt-6 rounded-2xl bg-app px-4 py-3 text-xs text-ink-300">Escolha seu ambiente...</div>
